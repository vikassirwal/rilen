...
WORKBOOK_PATH = Path("/Users/vikassirwal/Downloads/6:7:8final_spreadsheet.xlsx")
OUTPUT_PATH = Path("db/student_import_class_6_7_8.sql")
...
def is_header_row(row: tuple[Any, ...]) -> bool:
    return normalize_header(row[0]) == "S.NO"
    ...
    for worksheet in workbook.worksheets:
        ...
        for row_number, row in enumerate(worksheet.iter_rows(values_only=True), start=1):
            ...
            for index, header in enumerate(headers):
                if header is None or header == "S.NO":
                    continue
...
def registration_insert(record: dict[str, Any]) -> str:
    mapped = record["mapped"]
    tc_status = "LEFT" if record["section"] == "left" else mapped.get("tc_status")
...
def main() -> None:
    ...
    print(json.dumps({"output": str(OUTPUT_PATH), "records": len(records), "counts": counts}, indent=2))
