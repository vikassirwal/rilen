...
WORKBOOK_PATH = Path("/Users/vikassirwal/Desktop/nur-ukg.xlsx")
OUTPUT_PATH = Path("db/student_import_class_nur_ukg.sql")
ACADEMIC_YEAR = "2026-27"

NULL_TOKENS = {"", "?", "NA", "N/A", "#REF!"}
EMPTY_PREFIX_TOKENS = {"A-", "AC-", "M-", "SBI-", "D-"}
CLASS_BY_SHEET = {
    "NUR": "NUR",
    "LKG": "LKG",
    "UKG": "UKG",
...
def clean_cell(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        return None if cleaned.upper() in NULL_TOKENS or compact in EMPTY_PREFIX_TOKENS or "?" in cleaned else cleaned
...
def sql_identifier_text(value: Any) -> str:
    cleaned = text_value(value)
    if cleaned is None:
        return "null"
    return "'" + cleaned.replace("'", "''") + "'"
...
def normalize_record_values(record: dict[str, Any]) -> None:
    mapped = record["mapped"]
    scholar_number = text_value(mapped.get("scholar_number"))
    admission_number = text_value(mapped.get("admission_number"))
    if scholar_number and admission_number and scholar_number == admission_number:
        mapped["scholar_number"] = None
        record["corrections"].append("Set scholar number to null because source duplicated admission number in SCH. column.")


def resolve_duplicate_scholars(records: list[dict[str, Any]]) -> None:
    ...
    for record in records:
        ...
        if key in seen:
            record["mapped"]["scholar_number"] = None
            first = seen[key]
            record["corrections"].append(
                f"Set duplicate scholar number {scholar} to null; first seen at {first['sheet']} row {first['row_number']}."
            )
        else:
            seen[key] = record
...
def extract_records() -> list[dict[str, Any]]:
    ...
    for worksheet in workbook.worksheets:
        ...
        for row_number, row in enumerate(worksheet.iter_rows(values_only=True), start=1):
            ...
            normalize_record_values(record)
            records.append(record)

    resolve_duplicate_scholars(records)
    return records
...
def student_insert(record: dict[str, Any]) -> str:
  ...
  {sql_identifier_text(mapped.get("aadhaar_number"))},
  {sql_identifier_text(mapped.get("sssm_id"))},
  {sql_identifier_text(mapped.get("family_id"))},
